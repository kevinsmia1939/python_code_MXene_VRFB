...
for cycle in np.arange(0,4,1):
    low_range_peak, high_range_peak, peak_volt, peak_curr, low_range_trough, high_range_trough, trough_volt, trough_curr = get_CV_peak(volt_arr[cycle],current_arr[cycle],200,700,1600)
    
    jpa_lnfit = np.polyfit(volt_list[cycle][jpa_lns:jpa_lne],current_list[cycle][jpa_lns:jpa_lne], 1)
    jpa_base = jpa_lnfit[0]*peak_volt + jpa_lnfit[1]

    jpc_lnfit = np.polyfit(volt_list[cycle][jpc_lns:jpc_lne],current_list[cycle][jpc_lns:jpc_lne], 1)
    ...
    plt.plot((volt_arr[cycle][jpa_lns],peak_volt),(current_arr[cycle][jpa_lns],jpa_base),'--')
    plt.plot((volt_arr[cycle][jpc_lns],trough_volt),(current_arr[cycle][jpc_lns],jpc_base),'--')
...
scan_rate = np.array([3,5,10,20])
scan_rate_sqrt = scan_rate**0.5
plt.plot(scan_rate_sqrt,jpa_list,'^',label='$V^{4+}$ → $V^{5+}$')
plt.plot(scan_rate_sqrt,-jpc_list,'o',label='$V^{5+}$ → $V^{4+}$')

lin_fit_jpa = np.polyfit(scan_rate_sqrt,jpa_list, 1)
lin_fit_jpc = np.polyfit(scan_rate_sqrt,-jpc_list, 1)
# lin_fit_jpc = np.polyfit(number_cycle,-jpc_list, 1)
# plt.plot(scan_rate_sqrt,scan_rate_sqrt)
plt.plot(scan_rate_sqrt,lin_fit_jpa[0]*scan_rate_sqrt+lin_fit_jpa[1])
plt.plot(scan_rate_sqrt,lin_fit_jpc[0]*scan_rate_sqrt+lin_fit_jpc[1])

plt.xlabel('Scan rate (mV/s)$^{1/2}$', fontsize=16)
...
# fig, ax = plt.subplots(figsize=(8,8))
# plt.plot(scan_rate,np.array(peak_sep_list)*1000,'o')
