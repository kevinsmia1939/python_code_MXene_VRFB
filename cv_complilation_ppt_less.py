...
def cv_peak_sqrt(title,a,b, file_name1,file_name2,scan_list,elec_area,jpa_lns,jpa_lne,jpc_lns,jpc_lne,peak_range, peak_pos, trough_pos):
    for i in scan_list:
        ...
        ax[a].plot(volt,current,'-',linewidth=2.5,label=title)
        ax[a].set_xlabel('Voltage (V) vs Ag/AgCl')
        ax[a].set_ylabel('Current density (mA/cm$^2$)')

        # legend_patch = mpatches.Rectangle((0, 0), 1, 1, facecolor='red', edgecolor='black')
        # ax[a].add_patch(legend_patch)
        # legend = ax.legend([legend_patch], ['Legend'], loc='upper right')
        if a == 0:
            ax[a].set_xlim(-0.75,0)
        if a == 1:
            ax[a].set_xlim(0,1.2)
        if a == 0 and b == 0:
            ax[a].text(-0.4, -3, 'Scan rate: 4 mV/s', color='black',horizontalalignment='center',verticalalignment='center', fontsize=20)
            ax[a].legend(fontsize=16)
        if a == 1 and b == 0:
            ax[a].text(0.2, 2.7, 'Scan rate: 5 mV/s', color='black',horizontalalignment='center',verticalalignment='center', fontsize=20)
            ax[a].legend(fontsize=16)
        # elif b == 1:
        #     ax[a].set_xlim(0,1.2)
        # elif a == 1:
        ...
        # else:
        #     ax[a].set_xlim(0,1.2)
        # ax[a].set_title(title,y=1,x=0.05,pad=-14)
        
        # if b==1 and a==1:
        #     ax[a].set_yticks([-10,-8,-6,-4,-2,0,2,4,6,8,10,12])
...
trough_pos = 900
a,b = 0,0
title = 'Heat-treated carbon paper'
cv_peak_sqrt(title,a,b,file_name1,file_name2,scan_list,elec_area,jpa_lns,jpa_lne,jpc_lns,jpc_lne,peak_range, peak_pos, trough_pos)
...
trough_pos = 740
a,b = 0,0
title = 'MXene-coated carbon paper (0.1 mg/cm$^2$)'
cv_peak_sqrt(title,a,b,file_name1,file_name2,scan_list,elec_area,jpa_lns,jpa_lne,jpc_lns,jpc_lne,peak_range, peak_pos, trough_pos)
...
trough_pos = 740
a,b = 0,0
title = 'MXene-coated carbon paper (0.5 mg/cm$^2$)'
cv_peak_sqrt(title,a,b,file_name1,file_name2,scan_list,elec_area,jpa_lns,jpa_lne,jpc_lns,jpc_lne,peak_range, peak_pos, trough_pos)
