...
color_lst = ["red","lightblue","orange","orchid"]
mark = ["s","^","v","D"]
bat_file = ["HT|HT", "MX-0.1|HT",  "MX-0.1|MX-0.1",  "MX-0.5|MX-0.5"]
leg = ["Heat-treated |Heat-treated", "MXene-coated (0.1mg/cm$^2$)|Heat-treated",  "MXene-coated (0.1mg/cm$^2$)|MXene-coated (0.1mg/cm$^2$)",  "MXene-coated (0.5mg/cm$^2$)|MXene-coated (0.5mg/cm$^2$)"]
col_idx = 0
ax[0,0].plot(np.NaN,np.NaN,linestyle='',label="Negative | Positive")
for i in bat_file:
    file = "battery_ods/" + str(i) +".ods"
    df = pd.read_excel(file, engine="odf")
    ax[0,0].plot(df.loc[1:,'cycle number'],df.loc[1:,'charge-capacity'], label=leg[col_idx], linestyle='', marker=mark[col_idx], markerfacecolor=color_lst[col_idx], markeredgecolor='black',markeredgewidth=0.5,markersize=6)
    col_idx += 1

ax[0,0].legend(fontsize=15,loc = "upper left", bbox_to_anchor=(-1.8,0.2), markerscale=2)
...
for i in bat_file:
    file = "battery_ods/" + str(i) +".ods"
    df = pd.read_excel(file, engine="odf")
    ax[0,1].plot(df.loc[1:,'cycle number'],df.loc[1:,'discharge-capacity'], label=str(i), linestyle='', marker=mark[col_idx], markerfacecolor=color_lst[col_idx], markeredgecolor='black',markeredgewidth=0.5,markersize=6)
...
for i in bat_file:
    file = "battery_ods/" + str(i) +".ods"
    df = pd.read_excel(file, engine="odf")
    ax[1,0].plot(df.loc[1:,'cycle number'],df.loc[1:,'ee'], label=str(i), linestyle='', marker=mark[col_idx], markerfacecolor=color_lst[col_idx], markeredgecolor='black',markeredgewidth=0.5,markersize=6)
    col_idx += 1
